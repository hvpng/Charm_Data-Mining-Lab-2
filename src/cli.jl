...
const USAGE = """
Usage:
    julia --project=. src/cli.jl --input <path> --output <path> --minsup <int|float> [--impl basic|bitset]
"""
...
# Returns
- `Dict`: Parsed option map containing `--input`, `--output`, `--minsup`, and `--impl`.
...
function parse_args(args::Vector{String})
    ...
    while i <= length(args)
        key = args[i]
        haskey(opts, key) || error("Unknown argument: $key. Valid options are: --input, --output, --minsup, --impl\n$USAGE")
...
function main(args=ARGS)
    ...
    result = charm(txns, minsup; implementation=impl)
    write_spmf_itemsets(result, opts["--output"])
    println("Done. transactions=$(result.n_transactions), minsup=$(result.min_support), itemsets=$(length(result)), impl=$(impl)")
end
